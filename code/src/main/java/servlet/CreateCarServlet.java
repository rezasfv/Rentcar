 ...
 * @since 1.0
 */
public class CreateCarServlet extends AbstractDatabaseServlet{
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException{
        ...
        try{
            ...
            new CreateCarDAO(getConnection(), car).access();

        }
        catch(SQLException ex){
            if ("23505".equals(ex.getSQLState())) {
                message = new Message("The car already exists in the database.");
                LOGGER.error("The car already exists in the database.");
            } else {
                message = new Message("Unable to create the car: SQL exception");
                LOGGER.error("Unable to create the car: SQL exception", ex);
            }
        }
        catch(NullPointerException e){
            message = new Message("Unable to create the car: null pointer exception");
            LOGGER.error("Unable to create the car: null pointer exception", e);
        }
